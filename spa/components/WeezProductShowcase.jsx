...
export default function WeezProductShowcase() {
    ...
    const inView = useInView(sectionRef, { margin: "-10% 0px", amount: 0.25 });
    const prefersReduced = useReducedMotion();
    const isFinePointer =
        typeof window !== "undefined" &&
        window.matchMedia &&
        window.matchMedia("(pointer:fine)").matches;
    const effectsEnabled = inView && !prefersReduced && isFinePointer;

    return (
        <section
            ref={sectionRef}
            className="relative isolate overflow-hidden bg-black text-[#EEE7D2]"
        >
            ...
            <div className="mx-auto max-w-[80rem] px-4 py-20 sm:py-28">
                <motion.div
                    initial={prefersReduced ? false : { opacity: 0, y: 8, filter: "blur(6px)" }}
          ...
          <span className="inline-flex items-center gap-2 rounded-full border border-[#2b2b2b] bg-black/60 px-3 py-1 text-xs tracking-widest text-[#EEE7D2]/80">
            ...
            <AnimatePresence mode="wait">
              ...
              >
                ...
                >
                    Weez AI connects your docs, code, and conversations to deliver cited
                    answers, smart automations, and zero-friction teamwork — in one place.
                </motion.p>
                ...
                <div className="mt-10 grid gap-6 lg:grid-cols-2">
                    ...
                    <div className="grid content-start gap-4 sm:grid-cols-2">
                        <TiltCard enabled={effectsEnabled}>
                            <CardIcon Icon={FolderSearch} />
                            <CardTitle>Unified knowledge</CardTitle>
                            <CardText>
                                Index Drive, Notion, GitHub, mail &amp; more. Ask once — get
                                cited answers.
                            </CardText>
                        </TiltCard>
                        <TiltCard enabled={effectsEnabled}>
                            ...
                            <CardText>
                                Triggers from commits, forms or messages; route approvals; push
                                updates.
                            </CardText>
                        </TiltCard>
                        <TiltCard enabled={effectsEnabled}>
                            ...
                            <CardText>
                                Granular permissions, auditability, and least-privilege access.
                            </CardText>
                        </TiltCard>
                        <TiltCard enabled={effectsEnabled}>
                            <CardIcon Icon={Timer} />
                            ...
                            <CardText>
                                Low-latency answers and near-realtime summaries for your team.
                            </CardText>
                        </TiltCard>
                    </div>
                </div>
                ...
                <div className="mt-12 flex flex-col items-center justify-between gap-6 border-t border-[#2b2b2b] pt-8 sm:flex-row">
                    ...
                    <div className="flex items-center gap-3">
                        <Magnetic enabled={effectsEnabled}>
                            <motion.a
                        ...
                        <Magnetic enabled={effectsEnabled} strength={0.25}>
                            <motion.a
        ...
        >
            <div className="grid gap-3 sm:grid-cols-2">
                <ResultCard
                    icon={BookOpenText}
                    title="Strategy.pdf"
                    meta="Found in /Drive/OKRs"
                />
                <ResultCard
                    icon={FolderSearch}
                    title="Onboarding Notion"
                    meta="Workspace › Docs"
                />
                <ResultCard
                    icon={Search}
                    title="Cited answer"
                    meta="3 sources · 97% confidence"
                    highlight
                />
    ...
    return (
        <div
            className={`max-w-[36rem] rounded-2xl border px-3 py-2 text-sm sm:px-4 sm:py-3 ${
                isWeez
                    ? "ml-0 border-[#334]/70 bg-white/5"
                    : "ml-auto border-[#2b2b2b] bg-black/50 text-[#EEE7D2]"
            }`}
        >
...
function FlowConnector({ vertical = false }) {
    return (
        <div
            className={`mx-auto h-0.5 w-8 rounded-full bg-[#2b2b2b] sm:w-10 ${
                vertical ? "col-span-3 h-8 w-0.5 sm:h-10" : ""
            }`}
        />
    );
}

function TiltCard({ children, enabled = true }) {
...
function CardTitle({ children }) {
    return <div className="text-sm font-semibold text-[#EEE7D2]">{children}</div>;
}
function CardText({ children }) {
    return <div className="mt-1 text-xs text-[#EEE7D2]/70">{children}</div>;
}

function Dot() {
    return <span className="h-1 w-1 rounded-full bg-[#EEE7D2]/50" />;
}
...
function Magnetic({ children, strength = 0.45, enabled = true }) {
    const ref = React.useRef(null);
